...
//        return facingSphereBlocks;
    ...
    public static Set<Block> getTopCircleBlocks(CoreAbility coreAbility) {
        return Blocks.getBlocksAroundPoint(coreAbility.getEntity().getLocation(), coreAbility.getRadius()).stream().filter(b -> Blocks.isTopBlock(b) && !b.isPassable()).collect(Collectors.toSet());
    }

    public static Set<Block> getTopCircleBlocksFloor(CoreAbility coreAbility) {

        Location floorLoc = Blocks.getBelowBlock(coreAbility.getEntity().getLocation().getBlock(), 100).getLocation();
