...
public class Blocks {
    ...
    public static boolean isBelowArchetype(Block b, double distance, SereneEntity serenityEntity) {
        Location loc = b.getLocation();
        if (loc.getWorld().rayTraceBlocks(loc, new Vector(0, -1, 0), distance, FluidCollisionMode.ALWAYS) != null) {
            ...
            return Blocks.getArchetypeBlocks(serenityEntity).contains(block.getType());
        }
        return false;
    }

    public static Block getFacingBlock(Entity entity, double distance) {
        Location loc = entity.getLocation().add(0,entity.getHeight()-0.5,0).clone();
        Block block = null;
        if (loc.getWorld().rayTraceBlocks(loc, loc.getDirection().subtract(new Vector (0,1,0)), distance, FluidCollisionMode.NEVER, true) != null) {
            RayTraceResult rayTraceResult = loc.getWorld().rayTraceBlocks(loc, loc.getDirection().subtract(new Vector (0,1,0)), distance, FluidCollisionMode.NEVER, true);
    ...
    public static Location getFacingBlockLoc(Entity entity, double distance) {
        Location loc = entity.getLocation().add(0,entity.getHeight()-0.5,0).clone();
        if (loc.getWorld().rayTraceBlocks(loc, loc.getDirection().subtract(new Vector (0,1,0)), distance, FluidCollisionMode.NEVER, true) != null) {
            RayTraceResult rayTraceResult = loc.getWorld().rayTraceBlocks(loc, loc.getDirection().subtract(new Vector (0,1,0)), distance, FluidCollisionMode.NEVER, true);
    ...
    public static boolean entityLookingAtBlockDisplay(Entity entity, BlockDisplay target, double maxDistance, double size) {
        Location loc = target.getLocation();

        Location lowest = loc.clone().add(Vectors.getDown(loc, size / 2)).add(Vectors.getLeftSide(entity, size / 2)).subtract(loc.getDirection().multiply(size / 2));
//        Particles.spawnParticle(Particle.FLAME, lowest, 1, 0, 0);
        Location highest = loc.clone().add(Vectors.getUp(loc, size / 2)).add(Vectors.getRightSide(entity, size / 2)).add(loc.getDirection().multiply(size / 2));
//        Particles.spawnParticle(Particle.FLAME, highest, 1, 0, 0);
        ...
        Location entityLoc = entity.getLocation().add(0,entity.getHeight()-0.5,0).clone();
        Vector dir = entity.getLocation().add(0,entity.getHeight()-0.5,0).getDirection().clone().normalize();
        RayTraceResult rayTraceResult = boundingBox.rayTrace(entityLoc.toVector(), dir, maxDistance);
    ...
    public static Block getFacingBlockOrLiquid(Entity entity, double distance) {
        Location loc = entity.getLocation().add(0,entity.getHeight()-0.5,0).clone();
        Block block = null;
        if (loc.getWorld().rayTraceBlocks(loc, loc.getDirection().subtract(new Vector (0,1,0)), distance, FluidCollisionMode.ALWAYS) != null) {
            block = loc.getWorld().rayTraceBlocks(loc, loc.getDirection().subtract(new Vector (0,1,0)), distance, FluidCollisionMode.ALWAYS).getHitBlock();
        }
        return block;
    }
    ...
    public static Location getFacingBlockOrLiquidLoc(Entity entity, double distance) {
        Location loc = entity.getLocation().add(0,entity.getHeight()-0.5,0).clone();
        if (loc.getWorld().rayTraceBlocks(loc, loc.getDirection().subtract(new Vector (0,1,0)), distance, FluidCollisionMode.ALWAYS) != null) {
            RayTraceResult rayTraceResult = loc.getWorld().rayTraceBlocks(loc, loc.getDirection().subtract(new Vector (0,1,0)), distance, FluidCollisionMode.ALWAYS);
    ...
    public static Set<Material> getArchetypeBlocks(SereneEntity serenityEntity) {
        return ArchetypeDataManager.getArchetypeData(serenityEntity.getArchetype()).getBlocks();
    }

    public static Block getSourceBlock(Entity entity, SereneEntity sEntity, double sourceRange) {
